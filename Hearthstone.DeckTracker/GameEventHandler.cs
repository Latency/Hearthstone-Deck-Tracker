...
using HearthStone.DeckTracker.API;
...
using HearthStone.DeckTracker.Live;
using HearthStone.DeckTracker.Replay;
...
using HearthStone.DeckTracker.Utility.Toasts;
using HearthStone.DeckTracker.Windows;
using HearthStone.Mirror;
using HearthStone.Replay.LogValidation;
...
namespace HearthStone.DeckTracker
{
	public class GameEventHandler : IGameHandler
	{
		...
		public bool UploadCurrentGameMode => _game.CurrentGameMode == GameMode.Practice && Config.Instance.HsReplayUploadPractice
											 || _game.CurrentGameMode == GameMode.Arena && Config.Instance.HsReplayUploadArena
											 || _game.CurrentGameMode == GameMode.Brawl && Config.Instance.HsReplayUploadBrawl
											 || _game.CurrentGameMode == GameMode.Ranked && Config.Instance.HsReplayUploadRanked
											 || _game.CurrentGameMode == GameMode.Friendly && Config.Instance.HsReplayUploadFriendly
											 || _game.CurrentGameMode == GameMode.Casual && Config.Instance.HsReplayUploadCasual
											 || _game.CurrentGameMode == GameMode.Spectator && Config.Instance.HsReplayUploadSpectator;
		...
		public void HandleInMenu()
		{
			...
			if(!_game.IsUsingPremade)
				_game.DrawnLastGame =
					...
					{
						var card = Hearthstone.Database.GetCardFromId(x.Key);
						card.Count = x.Count();
						return card;
					}));
		...
		{
			if(LogContainsGoldRewardState || _game.CurrentGameMode == GameMode.Practice && LogContainsStateComplete)
				return;
		...
		{
			var hero = Hearthstone.Database.GetHeroNameFromId(cardId);
		...
		{
			var thaurissans = _game.Opponent.Board.Where(x => x.CardId == Database.CardIds.Collectible.Neutral.EmperorThaurissan && !x.HasTag(GameTag.SILENCED)).ToList();
			if(!thaurissans.Any())
				return;
		...
		{
			if(_game.CurrentGameMode == GameMode.Practice && !_game.IsInMenu && !_handledGameEnd
...
#pragma warning disable 4014
		public async void HandleGameEnd()
		{
			try
			{
				...
				if(_game.CurrentGameMode == GameMode.Spectator && _game.CurrentGameStats.Result == GameResult.None)
				{
				...
				var opponent = _game.Entities.FirstOrDefault(e => e.Value != null && e.Value.HasTag(GameTag.PLAYER_ID) && !e.Value.IsPlayer).Value;
				if(player != null)
				{
					_game.CurrentGameStats.PlayerName = player.Name;
					_game.CurrentGameStats.Coin = !player.HasTag(GameTag.FIRST_PLAYER);
				}
				...
				if(_game.CurrentGameMode == GameMode.Ranked && _game.MatchInfo != null)
				{
				...
				else if(_game.CurrentGameMode == GameMode.Arena)
				{
				...
				else if(_game.CurrentGameMode == GameMode.Brawl && _game.BrawlInfo != null)
				{
...
#pragma warning restore 4014
		...
		private void SaveAndUpdateStats()
		{
			if(RecordCurrentGameMode)
			{
				...
				if(_game.CurrentGameStats != null)
				{
					...
					if(_game.CurrentGameStats.GameMode == GameMode.Arena)
					{
						ArenaStats.Instance.UpdateArenaStats();
						ArenaStats.Instance.UpdateArenaRuns();
						ArenaStats.Instance.UpdateArenaStatsHighlights();
		...
		public void HandlePlayerGetToDeck(Entity entity, string cardId, int turn)
		{
			...
			GameEvents.OnPlayerCreateInDeck.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandlePlayerGet(Entity entity, string cardId, int turn)
		{
			...
			Core.UpdatePlayerCards();
			GameEvents.OnPlayerGet.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandlePlayerBackToHand(Entity entity, string cardId, int turn)
		{
			...
			GameEvents.OnPlayerPlayToHand.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandlePlayerDraw(Entity entity, string cardId, int turn)
		{
			...
			GameEvents.OnPlayerDraw.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandlePlayerMulligan(Entity entity, string cardId)
		{
			...
			Core.UpdatePlayerCards();
			GameEvents.OnPlayerMulligan.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandlePlayerSecretPlayed(Entity entity, string cardId, int turn, Zone fromZone)
		{
			...
			if(!entity.IsSecret)
			{
				if(entity.IsQuest)
				{
					...
					GameEvents.OnPlayerPlay.Execute(Hearthstone.Database.GetCardFromId(cardId));
				}
				return;
			}
		...
		public void HandlePlayerHandDiscard(Entity entity, string cardId, int turn)
		{
			...
			Core.UpdatePlayerCards();
			GameEvents.OnPlayerHandDiscard.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandlePlayerDeckDiscard(Entity entity, string cardId, int turn)
		{
			...
			DeckManager.DetectCurrentDeck().Forget();
			Core.UpdatePlayerCards();
			GameEvents.OnPlayerDeckDiscard.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandlePlayerPlayToDeck(Entity entity, string cardId, int turn)
		{
			...
			Core.UpdatePlayerCards();
			GameEvents.OnPlayerPlayToDeck.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandlePlayerCreateInPlay(Entity entity, string cardId, int turn)
		{
			...
			GameEvents.OnPlayerCreateInPlay.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}

		public void HandleOpponentCreateInPlay(Entity entity, string cardId, int turn)
		{
			_game.Opponent.CreateInPlay(entity, turn);
			GameEvents.OnOpponentCreateInPlay.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandlePlayerJoust(Entity entity, string cardId, int turn)
		{
			...
			Core.UpdatePlayerCards();
			GameEvents.OnPlayerJoustReveal.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandlePlayerDeckToPlay(Entity entity, string cardId, int turn)
		{
			...
			Core.UpdatePlayerCards();
			GameEvents.OnPlayerDeckToPlay.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandleOpponentDeckToPlay(Entity entity, string cardId, int turn)
		{
			...
			Core.UpdateOpponentCards();
			GameEvents.OnOpponentDeckToPlay.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandlePlayerStolen(Entity entity, string cardId, int turn)
		{
			...
			{
				HeroClass heroClass;
				var className = ((CardClass)entity.GetTag(GameTag.CLASS)).ToString();
		...
		public void HandleOpponentStolen(Entity entity, string cardId, int turn)
		{
			...
			if(entity.IsSecret)
			{
				...
				Core.UpdateOpponentCards();
				GameEvents.OnOpponentSecretTriggered.Execute(Hearthstone.Database.GetCardFromId(cardId));
			}
		}
		...
		public void HandleOpponentPlay(Entity entity, string cardId, int from, int turn)
		{
			...
			Core.UpdateOpponentCards();
			GameEvents.OnOpponentPlay.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandleOpponentJoust(Entity entity, string cardId, int turn)
		{
			...
			Core.UpdateOpponentCards();
			GameEvents.OnOpponentJoustReveal.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandleOpponentHandDiscard(Entity entity, string cardId, int from, int turn)
		{
			...
			Core.UpdateOpponentCards();
			GameEvents.OnOpponentHandDiscard.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		{
			if(!_game.IsMulliganDone && entity.GetTag(GameTag.ZONE_POSITION) == 5)
				entity.CardId = Database.CardIds.NonCollectible.Neutral.TheCoin;
		...
		public void HandleOpponentPlayToHand(Entity entity, string cardId, int turn, int id)
		{
			...
			Core.UpdateOpponentCards();
			GameEvents.OnOpponentPlayToHand.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandleOpponentPlayToDeck(Entity entity, string cardId, int turn)
		{
			...
			Core.UpdateOpponentCards();
			GameEvents.OnOpponentPlayToDeck.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}
		...
		public void HandleOpponentDeckDiscard(Entity entity, string cardId, int turn)
		{
			...
			Core.UpdateOpponentCards();
			GameEvents.OnOpponentDeckDiscard.Execute(Hearthstone.Database.GetCardFromId(cardId));
		}

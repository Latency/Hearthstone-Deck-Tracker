	...
	public partial class OverlayWindow
	{
		...
		public void Update(bool refresh)
		{
			...
			for (var i = 0; i < 10; i++)
			{
				...
				{
					var entity = _game.Opponent.Hand.FirstOrDefault(x => x.GetTag(GameTag.ZONE_POSITION) == i + 1);
					if(entity == null)
						continue;
			...
			var playerBoard = Core.Game.Player.Board.Where(x => x.IsMinion).OrderBy(x => x.GetTag(GameTag.ZONE_POSITION)).ToList();
		...
		private void UpdateIcons()
		{
			...
			if (_game.Entities.Count > 67 && (IconBoardAttackPlayer.Visibility == Visibility.Visible || IconBoardAttackOpponent.Visibility == Visibility.Visible))
			{
			...
			WotogIconsPlayer.WotogCounterStyle = showPlayerCthunCounter && showPlayerSpellsCounter ? WotogCounterStyle.Full : (showPlayerCthunCounter ? WotogCounterStyle.Cthun : (showPlayerSpellsCounter ? WotogCounterStyle.Spells : WotogCounterStyle.None));
			WotogIconsPlayer.JadeCounterStyle = showPlayerJadeCounter ? WotogCounterStyle.Full : WotogCounterStyle.None;
			...
			WotogIconsOpponent.WotogCounterStyle = showOpponentCthunCounter && showOpponentSpellsCounter ? WotogCounterStyle.Full : (showOpponentCthunCounter ? WotogCounterStyle.Cthun : (showOpponentSpellsCounter ? WotogCounterStyle.Spells : WotogCounterStyle.None));
			WotogIconsOpponent.JadeCounterStyle = showOpponentJadeCounter ? WotogCounterStyle.Full : WotogCounterStyle.None;
		...
		public void UpdatePosition()
		{
			...
			if (hsRect.Height == 0 || (Visibility != Visibility.Visible && Core.Windows.CapturableOverlay == null))
				return;

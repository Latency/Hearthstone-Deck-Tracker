...
namespace HearthStone.DeckTracker.Controls.Stats.Constructed
{
	/// <summary>
	...
	public partial class ConstructedGames : INotifyPropertyChanged
	{
		...
		private async void ButtonEditNote_OnClick(object sender, RoutedEventArgs e)
		{
			...
			string newNote;
			if(Config.Instance.StatsInWindow)
				newNote = await DialogManager.ShowInputAsync(Core.Windows.StatsWindow, "Note", "", settings);
			else
				newNote = await DialogManager.ShowInputAsync(Core.MainWindow, "Note", "", settings);
			if(newNote == null)
				return;

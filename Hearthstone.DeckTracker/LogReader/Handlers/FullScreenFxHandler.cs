...
namespace HearthStone.DeckTracker.LogReader.Handlers
{
	public class FullScreenFxHandler
	{
		...
		public void Handle(LogLine logLine, IGame game)
		{
			...
			if(match.Success && game.IsInMenu && new[] {Mode.TAVERN_BRAWL, Mode.TOURNAMENT, Mode.DRAFT, Mode.FRIENDLY, Mode.ADVENTURE}.Contains(game.CurrentMode))
			{
				...
				if(!Config.Instance.AutoDeckDetection)
					return;
				if(new[] {Mode.TOURNAMENT, Mode.FRIENDLY, Mode.ADVENTURE, Mode.TAVERN_BRAWL}.Contains(game.CurrentMode))
		...
		private static async void AutoSelectDeckById(IGame game)
		{
			...
			DeckManager.AutoImportConstructed(false, game.CurrentMode == Mode.TAVERN_BRAWL);

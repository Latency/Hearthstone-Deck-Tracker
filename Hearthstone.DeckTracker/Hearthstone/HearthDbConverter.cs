...
namespace HearthStone.DeckTracker.Hearthstone
{
	public static class HearthDbConverter
	{
		...
		public static BnetGameType GetGameType(GameMode mode, Format? format)
		{
			switch(mode)
			{
				...
				case GameMode.Ranked:
					return format == Format.Standard ? BnetGameType.BGT_RANKED_STANDARD : BnetGameType.BGT_RANKED_WILD;
				case GameMode.Casual:
					return format == Format.Standard ? BnetGameType.BGT_CASUAL_STANDARD : BnetGameType.BGT_CASUAL_WILD;
				case GameMode.Brawl:
		...
		public static HearthStone.Database.Deckstrings.Deck ToHearthDbDeck(Deck deck)
		{
			var card = Database.GetHeroCardFromClass(deck.Class);
			if(card?.DbfIf > 0)
			{
				return new HearthStone.Database.Deckstrings.Deck
				{
					Name = deck.Name,
		...
		public static Deck FromHearthDbDeck(HearthStone.Database.Deckstrings.Deck hDbDeck)
		{
			var deck = new Deck
			{
				Name = hDbDeck.Name,

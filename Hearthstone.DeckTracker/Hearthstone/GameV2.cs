...
namespace HearthStone.DeckTracker.Hearthstone
{
	public class GameV2 : IGame
	{
		...
		public Mirror.Objects.Deck CurrentSelectedDeck { get; set; }
		...
		public Format? CurrentFormat
		{
			get
			{
				if(_currentFormat == FormatType.FT_UNKNOWN)
					_currentFormat = (FormatType)Mirror.Reflection.GetFormat();
		...
		public bool Spectator => _spectator ?? (bool)(_spectator = Mirror.Reflection.IsSpectating());
		...
		public GameType CurrentGameType => _currentGameType != GameType.GT_UNKNOWN ? _currentGameType : (_currentGameType = (GameType)Mirror.Reflection.GetGameType());

		public MatchInfo MatchInfo => _matchInfo ?? (_matchInfo = Mirror.Reflection.GetMatchInfo());
		private bool _matchInfoCacheInvalid = true;

		public BrawlInfo BrawlInfo => _brawlInfo ?? (_brawlInfo = Mirror.Reflection.GetBrawlInfo());
		...
		internal async void CacheMatchInfo()
		{
			...
			MatchInfo matchInfo;
			while((matchInfo = Mirror.Reflection.GetMatchInfo()) == null || matchInfo.LocalPlayer == null || matchInfo.OpposingPlayer == null)
		...
		{
			while((_currentGameType = (GameType)Mirror.Reflection.GetGameType()) == GameType.GT_UNKNOWN)
				await Task.Delay(1000);
		}

		internal void CacheSpectator() => _spectator = Mirror.Reflection.IsSpectating();

		internal void CacheBrawlInfo() => _brawlInfo = Mirror.Reflection.GetBrawlInfo();

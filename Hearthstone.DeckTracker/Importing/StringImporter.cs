		...
		public static Deck Import(string cards, bool localizedNames = false)
		{
			try
			{
				...
				foreach(var line in lines)
				{
					...
					var card = Hearthstone.Database.GetCardFromName(cardName.Replace("’", "'"), localizedNames);
					if(string.IsNullOrEmpty(card?.Name) || card.Id == Hearthstone.Database.UnknownCardId)
						continue;
					card.Count = count;
